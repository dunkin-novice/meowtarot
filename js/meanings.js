...
function renderMeaningList() {
  ...
  meowTarotCards.slice(0, 12).forEach((card) => {
    const name = state.currentLang === 'en' ? card.card_name_en : card.alias_th;
    const summary = state.currentLang === 'en' ? card.reading_summary_preview_en : card.reading_summary_preview_th;
    const archetype = state.currentLang === 'en' ? card.archetype_en : card.archetype_th;
    const div = document.createElement('div');
    div.className = 'sample-card';
    div.innerHTML = `<h5>${card.icon_emoji ? `${card.icon_emoji} ` : ''}${name}</h5><p class="archetype">${archetype}</p><p>${summary}</p>`;
