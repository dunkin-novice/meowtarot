...
function updateSeo(card) {
  ...
  const baseTitle = `${card.card_name_en || 'The Chariot'} Tarot Meaning (${orientationLabel}) | MeowTarot`;
      ...
      },
      {
        ...
        itemListElement: [
          ...
          },
          {
            '@type': 'ListItem',
            position: 3,
            name: card.card_name_en || 'The Chariot',
            item: pageUrl,
          },
        ],
      },
...
function render() {
  ...
  if (footerNext) {
    const fallbackNext = localize('/cards/strength/');
  ...
  dom.cardNameHeading.textContent = card.card_name_en || 'The Chariot';
