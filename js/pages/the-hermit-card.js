...
function updateSeo(card) {
  ...
  const baseTitle = `${card.card_name_en || 'The Hermit'} Tarot Meaning (${orientationLabel}) | MeowTarot`;
      ...
      },
      {
        ...
        itemListElement: [
          ...
          },
          {
            '@type': 'ListItem',
            position: 3,
            name: card.card_name_en || 'The Hermit',
            item: pageUrl,
          },
        ],
      },
...
function render() {
  ...
  if (footerNext) {
    const fallbackNext = localize('/tarot-card-meanings/wheel-of-fortune-tarot-meaning/');
  ...
  dom.cardNameHeading.textContent = card.card_name_en || 'The Hermit';
