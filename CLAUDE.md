...
- **Deck-switcher UI: post-switch repaint requirement.** `js/main.js:67` runs `staticCardBacks.forEach(applyCardBackBackground)` once at module init. Already-rendered `.card-back` DOM nodes will not pick up the new deck after `setActiveDeck()` fires. When the deck-switcher UI is built, the author MUST do one of: (a) call a `repaintCardBacks()` helper in `main.js` immediately after `setActiveDeck()` from the UI's click handler, OR (b) add a `subscribeActiveDeck(listener)` pattern in `js/data.js` and have `main.js` subscribe at module init so any future `setActiveDeck()` caller anywhere automatically triggers the repaint. (b) is forget-proof; (a) is smaller-diff but easy to miss in future call sites. Currently no callers of `setActiveDeck` exist (grep confirms), so the bug is theoretical until the UI lands — but DO NOT ship the UI without picking one. Context: deck-switching wiring landed 2026-05-13 to 2026-05-14 across `js/data.js`, `js/asset-resolver.js`, `js/main.js`.
