...
[2026-06-23 daily] D1: 8 missing-from-sitemap (live HEAD skipped), D2: 0 broken, D3: 0 issues, D4: 0 missing
  - CYCLE: Daily only. User-local date Tuesday 2026-06-23 (Thailand UTC+7; sandbox clock read 2026-06-22 ~18:02 UTC = early-AM Tue local per session env). Not Sunday -> no weekly; not 1st -> no monthly. Pre-flight OK: working dir present, brief present, log present.
  - D1 [ESCALATE]: Live HTTP HEAD checks for the sitemap <loc> URLs could not be performed. Per the operating-state network-routing directive I searched for the Composio remote bash tool by its current name (suffix COMPOSIO_REMOTE_BASH_TOOL / server prefix mcp__composio__) across two ToolSearch queries ("COMPOSIO_REMOTE_BASH_TOOL"; "composio remote bash internet HTTP request") -- NOT exposed in this session. Genuine continued outage (continuous since 2026-05-25; escalated repeatedly 06-09/10/11/12/18/19/20/21/22 and persists far past the 3-day threshold), not the dead-old-name confusion. Did NOT fall back to local curl/wget (prohibited by the network-routing directive). Dunkin: restore the Composio remote-bash connection OR amend the operating-state directive with a sanctioned fallback for live D1/W3 HEAD verification. Filesystem MISSING_FROM_SITEMAP check ran locally (clean-URL + canonical + noindex aware; 367 HTML files scanned).
  - D1: MISSING_FROM_SITEMAP -- 8 genuine findings (8 escalated; flag-only -- D1 has no auto-fix surface):
      - [ESCALATE] privacy.html (title "Privacy Policy"; tracked via git ls-files; no robots noindex; canonical = https://www.meowtarot.com/privacy.html, NOT in sitemap.xml -- grep "privacy" sitemap.xml -> 0). DAY 6 (first surfaced 2026-06-18; escalated since 06-20). Remains escalated.
      - [ESCALATE] th/privacy.html (TH Privacy Policy mirror; tracked; no robots noindex; canonical = https://www.meowtarot.com/th/privacy.html, also absent from sitemap.xml). DAY 6 -> remains escalated.
        To clear the privacy escalations permanently: either add both URLs to sitemap.xml (if you want them indexed) OR add `<meta name="robots" content="noindex">` to each (if intentionally non-indexed -- common for legal pages). D1 never edits sitemap.xml, so flag-only regardless.
      - [ESCALATE] share/poster-preview/index.html -- git-tracked. DAY 4 in tracked-and-flagging state (became tracked 2026-06-20; escalated since 06-22). Remains escalated.
      - [ESCALATE] share/poster-preview/daily-fit.html -- git-tracked. DAY 4 -> remains escalated.
      - [ESCALATE] share/poster-preview/celtic-harness.html -- git-tracked. DAY 4 -> remains escalated.
      - [ESCALATE] share/poster-preview/question-harness.html -- git-tracked. DAY 4 -> remains escalated.
      - [ESCALATE] share/poster-preview/question-harness-th.html -- git-tracked. DAY 4 -> remains escalated.
      - [ESCALATE] share/poster-preview/question-harness-single.html -- git-tracked. DAY 4 -> remains escalated.
    NOTE: the 6 share/poster-preview/*.html render-test harnesses remain GIT-TRACKED this cycle (confirmed via git ls-files share/poster-preview/). They sit under off-limits share/, so flag-only forever-until-acted regardless -- D1 has no auto-fix and never edits share/. Dunkin: these are internal poster render-test harnesses, almost certainly NOT meant to be indexed -- to silence permanently, add `<meta name="robots" content="noindex">` to each, OR `git rm --cached` them back to untracked, OR confirm intentional. No action taken.
  - D1 NEW-PAGE NOTE: HTML count rose 365 -> 367 since 2026-06-22. Two new tracked pages appeared: today/index.html and deck-detail.html. Both carry `<meta name="robots" content="noindex,follow">` (today/index.html canonical -> https://www.meowtarot.com/; deck-detail.html no canonical) and are therefore correctly EXCLUDED from MISSING_FROM_SITEMAP as intentionally non-indexed app/tool pages. They did not change the D1 count (still 8). Consistent with long-established noindex handling.
  - D1 FALSE-POSITIVE EXCLUSIONS (verified, NOT flagged): app/tool pages carrying `<meta name="robots" content="...noindex...">` (daily.html, full.html, question.html, reading.html, profile.html, decks.html, daily-card/, quiz/, today/, deck-detail.html, and th/ mirrors) correctly excluded via regex `content="[^"]*noindex`. question-draw.html / th/question-draw.html covered (clean URLs /question-draw and /th/question-draw present in sitemap). Consistent with long-established clean-URL + noindex handling.
  - D1 NOTE: tarot-card-meanings/card.html and th/tarot-card-meanings/card.html (known JS-driven template pages) treated as covered via canonical resolving to indexed sitemap entries; not flagged. Unchanged, still intentional.
  - D2: 367 HTML files scanned (excludes www/, ios/, node_modules/, .git/, tests/, scripts/, docs/, .log-draft-archive/, .claude/, .github/, cowork-scratch/, test-results/, sharekit/); all internal hrefs (/, https://meowtarot.com, https://www.meowtarot.com) checked against repo files (pretty-URL aware) + sitemap. 0 broken. Clean. The 3 previously-[KNOWN] /guides/* links remain ABSENT from meanings.html (grep "/guides/" -> 0); Dunkin removed them, so no [KNOWN] entry this cycle (consistent with 2026-06-09 onward).
  - D3: 78/78 EN card pages + 78/78 TH card pages = 156 pages PASS canonical + paired hreflang (en/en-XX, th/th-XX accepted per directive; order-independent attribute parsing). 0 CANONICAL_MISSING, 0 CANONICAL_WRONG, 0 HREFLANG_MISSING. Clean. D3 hreflang auto-fix had no work to apply; soft-start tracker remains "never applied to a hreflang" (the only prior [AUTO_FIX_APPLIED] was a share/index.html noindex, a different surface). Normal D3 auto-fix active this cycle (not in a pause window).
  - D4: 367 HTML files scanned, 24 <img> tags total, 0 missing alt attribute (robust multi-line <img> matching). Clean.
  - HELPERS: throwaway checker written to cowork-scratch/sweep-0623.py per operating-state directive; not persisted to scripts/.

[2026-06-24 daily] D1: 8 missing-from-sitemap (live HEAD skipped), D2: 0 broken, D3: 0 issues, D4: 0 missing
  - CYCLE: Daily only. User-local date Wednesday 2026-06-24 (Thailand UTC+7; sandbox clock read 2026-06-23 ~18:02 UTC = early-AM Wed local per session env). Not Sunday -> no weekly; not 1st -> no monthly. Pre-flight OK: working dir present, brief present, log present.
  - D1 [ESCALATE]: Live HTTP HEAD checks for the 174 sitemap <loc> URLs could not be performed. Per the operating-state network-routing directive I searched for the Composio remote bash tool by its current name (suffix COMPOSIO_REMOTE_BASH_TOOL / server prefix mcp__composio__) across two ToolSearch queries -- NOT exposed in this session. Genuine continued outage (continuous since 2026-05-25; escalated repeatedly 06-09 onward through 06-23 and persists far past the 3-day threshold), not the dead-old-name confusion. Did NOT fall back to local curl/wget (prohibited by the network-routing directive). Dunkin: restore the Composio remote-bash connection OR amend the operating-state directive with a sanctioned fallback for live D1/W3 HEAD verification. Filesystem MISSING_FROM_SITEMAP check ran locally (clean-URL + canonical + noindex aware; 367 HTML files scanned).
  - D1: MISSING_FROM_SITEMAP -- 8 genuine findings (8 escalated; flag-only -- D1 has no auto-fix surface):
      - [ESCALATE] privacy.html (title "Privacy Policy"; tracked via git ls-files; no robots noindex; canonical = https://www.meowtarot.com/privacy.html, NOT in sitemap.xml -- grep "privacy" sitemap.xml -> 0). DAY 7 (first surfaced 2026-06-18; escalated since 06-20). Remains escalated.
      - [ESCALATE] th/privacy.html (TH Privacy Policy mirror; tracked; no robots noindex; canonical = https://www.meowtarot.com/th/privacy.html, also absent from sitemap.xml). DAY 7 -> remains escalated.
        To clear the privacy escalations permanently: either add both URLs to sitemap.xml (if you want them indexed) OR add `<meta name="robots" content="noindex">` to each (if intentionally non-indexed -- common for legal pages). D1 never edits sitemap.xml, so flag-only regardless.
      - [ESCALATE] share/poster-preview/index.html -- git-tracked. DAY 5 in tracked-and-flagging state (became tracked 2026-06-20; escalated since 06-22). Remains escalated.
      - [ESCALATE] share/poster-preview/daily-fit.html -- git-tracked. DAY 5 -> remains escalated.
      - [ESCALATE] share/poster-preview/celtic-harness.html -- git-tracked. DAY 5 -> remains escalated.
      - [ESCALATE] share/poster-preview/question-harness.html -- git-tracked. DAY 5 -> remains escalated.
      - [ESCALATE] share/poster-preview/question-harness-th.html -- git-tracked. DAY 5 -> remains escalated.
      - [ESCALATE] share/poster-preview/question-harness-single.html -- git-tracked. DAY 5 -> remains escalated.
    NOTE: the 6 share/poster-preview/*.html render-test harnesses remain GIT-TRACKED this cycle (confirmed via git ls-files share/poster-preview/). They sit under off-limits share/, so flag-only forever-until-acted regardless -- D1 has no auto-fix and never edits share/. Dunkin: these are internal poster render-test harnesses, almost certainly NOT meant to be indexed -- to silence permanently, add `<meta name="robots" content="noindex">` to each, OR `git rm --cached` them back to untracked, OR confirm intentional. No action taken.
  - D1 NOTE: HTML count steady at 367 (no new pages since 2026-06-23). today/index.html and deck-detail.html remain correctly EXCLUDED via `<meta name="robots" content="noindex,follow">`.
  - D1 FALSE-POSITIVE EXCLUSIONS (verified, NOT flagged): app/tool pages carrying `<meta name="robots" content="...noindex...">` (daily.html, full.html, question.html, reading.html, profile.html, decks.html, daily-card/, quiz/, today/, deck-detail.html, and th/ mirrors) correctly excluded via regex `content="[^"]*noindex`. question-draw.html / th/question-draw.html covered (clean URLs present in sitemap). Consistent with long-established clean-URL + noindex handling.
  - D1 NOTE: tarot-card-meanings/card.html and th/tarot-card-meanings/card.html (known JS-driven template pages) treated as covered via canonical resolving to indexed sitemap entries; not flagged. Unchanged, still intentional.
  - D2: 367 HTML files scanned (excludes www/, ios/, node_modules/, .git/, tests/, scripts/, docs/, .log-draft-archive/, .claude/, .github/, cowork-scratch/, test-results/, sharekit/); all internal hrefs (/, https://meowtarot.com, https://www.meowtarot.com) checked against repo files (pretty-URL aware) + sitemap. 0 broken. Clean. The 3 previously-[KNOWN] /guides/* links remain ABSENT from meanings.html (grep "/guides/" -> 0); Dunkin removed them, so no [KNOWN] entry this cycle (consistent with 2026-06-09 onward).
  - D3: 78/78 EN card pages + 78/78 TH card pages = 156 pages PASS canonical + paired hreflang (en/en-XX, th/th-XX accepted per directive; order-independent attribute parsing). 0 CANONICAL_MISSING, 0 CANONICAL_WRONG, 0 HREFLANG_MISSING. Clean. D3 hreflang auto-fix had no work to apply; soft-start tracker remains "never applied to a hreflang" (the only prior [AUTO_FIX_APPLIED] was a share/index.html noindex, a different surface). Normal D3 auto-fix active this cycle (not in a pause window).
  - D4: 367 HTML files scanned, 24 <img> tags total, 0 missing alt attribute (robust multi-line <img> matching). Clean.
  - HELPERS: throwaway checker written to cowork-scratch/sweep-0624.py per operating-state directive; not persisted to scripts/.

[2026-06-25 daily] D1: 0 missing-from-sitemap (live HEAD skipped), D2: 0 broken, D3: 0 issues, D4: 0 missing
  - CYCLE: Daily only. User-local date Thursday 2026-06-25 (Thailand UTC+7; sandbox clock read 2026-06-24 ~?? UTC = early-AM Thu local per session env). Not Sunday -> no weekly; not 1st -> no monthly. Pre-flight OK: working dir present, brief present, log present.
  - RESOLVED (prior escalations cleared this cycle): All 8 standing MISSING_FROM_SITEMAP escalations are GONE. Dunkin added them to sitemap.xml:
      - privacy.html -> now <loc>https://www.meowtarot.com/privacy.html</loc> (sitemap.xml line 324). Cleared after DAY 7. RESOLVED.
      - th/privacy.html -> now <loc>https://www.meowtarot.com/th/privacy.html</loc> (line 696). RESOLVED.
      - share/poster-preview/ (index.html) -> covered by <loc>https://www.meowtarot.com/share/poster-preview/</loc> (line 332). RESOLVED.
      - share/poster-preview/celtic-harness.html (line 336), daily-fit.html (340), question-harness-single.html (344), question-harness-th.html (348), question-harness.html (352) -> all now explicit sitemap entries. RESOLVED.
    Net: Dunkin chose to INDEX the poster-preview render harnesses and the privacy pages rather than noindex them. Either resolution is acceptable per the brief; sitemap inclusion clears the flags. No further action.
  - D1 [ESCALATE]: Live HTTP HEAD checks for the 183 sitemap <loc> URLs could not be performed. Per the operating-state network-routing directive I searched for the Composio remote bash tool by its current name (suffix COMPOSIO_REMOTE_BASH_TOOL / server prefix mcp__composio__) across two ToolSearch queries ("COMPOSIO_REMOTE_BASH_TOOL"; "composio remote bash internet HTTP request") -- NOT exposed in this session. Genuine continued outage (continuous since 2026-05-25; escalated repeatedly 06-09 onward through 06-24 and persists far past the 3-day threshold), not the dead-old-name confusion. Did NOT fall back to local curl/wget (prohibited by the network-routing directive). Dunkin: restore the Composio remote-bash connection OR amend the operating-state directive with a sanctioned fallback for live D1/W3 HEAD verification. Filesystem MISSING_FROM_SITEMAP check ran locally (clean-URL + canonical + noindex aware; 369 HTML files scanned).
  - D1: MISSING_FROM_SITEMAP -- 0 findings (filesystem sweep). All previously-flagged tracked-and-indexable pages now covered by sitemap.xml. Clean.
  - D1 NOTE: sitemap <loc> count rose 174 -> 183 (9 new entries: privacy x2, widget x2, share/poster-preview dir + 5 harness files). HTML count rose 367 -> 369 since 2026-06-24: two new tracked pages widget.html and th/widget.html appeared; both ARE in sitemap.xml (lines 732, 728) so correctly NOT flagged (no canonical/robots meta, but sitemap inclusion satisfies D1). 
  - D1 FALSE-POSITIVE EXCLUSIONS (verified, NOT flagged): app/tool pages carrying `<meta name="robots" content="...noindex...">` (daily.html, full.html, question.html, reading.html, profile.html, decks.html, daily-card/, quiz/, today/, deck-detail.html, and th/ mirrors) correctly excluded. today/index.html, th/today/index.html, deck-detail.html, th/deck-detail.html confirmed carrying `content="noindex,follow"`. question-draw.html / th/question-draw.html covered (clean URLs present in sitemap). Consistent with long-established clean-URL + noindex handling.
  - D1 NOTE: tarot-card-meanings/card.html and th/tarot-card-meanings/card.html (known JS-driven template pages) treated as covered; not flagged. Unchanged, still intentional.
  - D2: 369 HTML files scanned (excludes www/, ios/, node_modules/, .git/, tests/, scripts/, docs/, .log-draft-archive/, .claude/, .github/, cowork-scratch/, test-results/, sharekit/); all internal hrefs (/, https://meowtarot.com, https://www.meowtarot.com) checked against repo files (pretty-URL aware) + sitemap. 0 broken. Clean. The 3 previously-[KNOWN] /guides/* links remain ABSENT from meanings.html; Dunkin removed them, so no [KNOWN] entry this cycle (consistent with 2026-06-09 onward).
  - D3: 78/78 EN card pages + 78/78 TH card pages = 156 pages PASS canonical + paired hreflang (en/en-XX, th/th-XX accepted per directive; order-independent attribute parsing). 0 CANONICAL_MISSING, 0 CANONICAL_WRONG, 0 HREFLANG_MISSING. Clean. D3 hreflang auto-fix had no work to apply; soft-start tracker remains "never applied to a hreflang" (the only prior [AUTO_FIX_APPLIED] was a share/index.html noindex, a different surface). Normal D3 auto-fix active this cycle (not in a pause window).
  - D4: 369 HTML files scanned, 34 <img> tags total, 0 missing alt attribute (robust multi-line <img> matching). Clean.
  - HELPERS: throwaway checker written to cowork-scratch/sweep-0625.py per operating-state directive; not persisted to scripts/.

[2026-06-26 daily] D1: 7 missing-from-sitemap (live HEAD skipped), D2: 0 broken, D3: 0 issues, D4: 0 missing
  - CYCLE: Daily only. User-local date Friday 2026-06-26 (Thailand UTC+7; sandbox clock read 2026-06-25 18:02 UTC = early-AM Fri local per session env). Not Sunday -> no weekly; not 1st -> no monthly. Pre-flight OK: working dir present, brief present, log present, brief read in full.
  - D1 [ESCALATE]: Live HTTP HEAD checks for the 177 sitemap <loc> URLs could not be performed. Per operating-state network-routing I searched for the Composio remote bash tool by its current suffix (COMPOSIO_REMOTE_BASH_TOOL, server prefix mcp__composio__) across two ToolSearch queries -- NOT exposed in this session. Genuine continued outage (continuous since 2026-05-25; escalated repeatedly 06-09 onward; persists far past the 3-day threshold). Did NOT fall back to local curl/wget (prohibited by the network-routing directive). Dunkin: restore the Composio remote-bash connection OR amend operating-state with a sanctioned fallback for live D1/W3 HEAD verification. Filesystem MISSING_FROM_SITEMAP check ran locally (clean-URL + canonical + noindex aware; 371 HTML files scanned).
  - D1 CONTEXT: sitemap.xml was edited by Dunkin 2026-06-25 14:46 (+0700) commit 2544c52 "refactor: consolidate widget distribution to single /widgets/ hub + one plugin". <loc> count fell 183 -> 177. The refactor migrated entries to CLEAN URLs (no .html) and DROPPED the share/poster-preview/* entries that had been added 06-25. Net effect on D1 below.
  - D1 MISSING_FROM_SITEMAP -- 7 findings (filesystem sweep), ALL the share/poster-preview/* render-test harnesses, re-emerged after one resolved cycle:
      - share/poster-preview/index.html -- git-tracked, no noindex.
      - share/poster-preview/celtic-harness.html -- git-tracked, no noindex.
      - share/poster-preview/celtic-harness-th.html -- git-tracked, no noindex. (NEW sibling; first appearance in this flag set.)
      - share/poster-preview/daily-fit.html -- git-tracked, no noindex.
      - share/poster-preview/question-harness.html -- git-tracked, no noindex.
      - share/poster-preview/question-harness-single.html -- git-tracked, no noindex.
      - share/poster-preview/question-harness-th.html -- git-tracked, no noindex.
    NOT escalated: these were RESOLVED last cycle (06-25, Dunkin had added them to sitemap), so the 3-day-running counter RESET. Today is day 1 of the re-occurrence after the 06-25 widget refactor removed them. They sit under off-limits share/ -> flag-only forever-until-acted regardless; D1 has no auto-fix and never edits share/. Dunkin: these are internal poster render-test harnesses, almost certainly NOT meant to be indexed -- to silence permanently, add `<meta name="robots" content="noindex">` to each, OR `git rm --cached` them back to untracked, OR re-add to sitemap. No action taken (correct per safety rules).
  - D1 FALSE-POSITIVE EXCLUSIONS (verified NOT flagged): privacy.html, th/privacy.html, question-draw.html, th/question-draw.html, widget.html, th/widget.html. The 06-25 refactor moved these to CLEAN-URL sitemap entries (/privacy, /th/privacy, /question-draw, /th/question-draw, /widget, /th/widget, plus /widgets/). Each flat .html file's canonical-equivalent clean URL IS present in sitemap.xml -> covered, not missing. (My first-pass helper over-flagged them because it only generated clean URLs for index.html files; verified by direct grep against sitemap.xml and excluded.)
  - D1 OBSERVATION (outside defined D-checks, no formal flag): widget.html AND th/widget.html both carry <link rel="canonical" href="https://www.meowtarot.com/widget.html"> -- the TH widget page's canonical points at the EN widget.html, and both point at the .html form rather than the new clean /widget URL now in the sitemap. D3's canonical/hreflang sweep is scoped to cards/* only, so this is not a D3 finding; surfacing as an observation for Dunkin in case the widget refactor left canonical drift. No action (out of scope to auto-fix canonicals).
  - D1 NOTE: tarot-card-meanings/card.html and th/tarot-card-meanings/card.html (known JS-driven template pages) treated as covered; not flagged. Unchanged, still intentional.
  - D2: 371 HTML files scanned (excludes www/, ios/, node_modules/, .git/, tests/, scripts/, docs/, .log-draft-archive/, .claude/, .github/, cowork-scratch/, test-results/, sharekit/); all internal hrefs (/, https://meowtarot.com, https://www.meowtarot.com) checked against repo files (pretty-URL + clean-URL + sitemap aware). 0 broken. Clean. The 3 previously-[KNOWN] /guides/* links remain ABSENT from meanings.html (grep "/guides/" -> 0); Dunkin removed them, so no [KNOWN] entry this cycle (consistent with 2026-06-09 onward).
  - D3: 78/78 EN card pages + 78/78 TH card pages = 156 pages PASS canonical + paired hreflang (en/en-XX, th/th-XX accepted per directive; order-independent attribute parsing). 0 CANONICAL_MISSING, 0 CANONICAL_WRONG, 0 HREFLANG_MISSING. Clean. D3 hreflang auto-fix had no work to apply; soft-start tracker remains "never applied to a hreflang" (the only prior [AUTO_FIX_APPLIED] was a share/index.html noindex, a different surface). Normal D3 auto-fix active this cycle (not in a pause window).
  - D4: 371 HTML files scanned, 0 <img> missing alt attribute (robust multi-line <img> matching). Clean.
  - HELPERS: throwaway checkers written to cowork-scratch/ (sweep-0626.py, d234-0626.py) per operating-state directive; not persisted to scripts/.
