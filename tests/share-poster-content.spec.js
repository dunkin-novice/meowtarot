...
test('share payload and poster render include card + reading content', async ({ page }) => {
  ...
  await page.goto(`${baseUrl}/reading.html`, { waitUntil: 'networkidle' });
  const warmedDeckSize = await page.evaluate(async () => {
    const data = await import('/js/data.js');
    await data.loadTarotData();
    return Array.isArray(data.meowTarotCards) ? data.meowTarotCards.length : 0;
  });
  diagnostics.rawConsole.push(`[deck_warmup] meowTarotCards=${warmedDeckSize}`);
  assert.ok(warmedDeckSize >= 150, `Expected warmed deck size >= 150, got ${warmedDeckSize}`);
