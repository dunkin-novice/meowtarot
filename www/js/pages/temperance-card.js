...
const CARD_ID_PREFIX = '15-temperance-upright';
...
function getPagePath(lang = 'en') {
  return getCanonicalCardPath('temperance', lang) || (lang === 'th' ? '/th/cards/temperance/' : '/cards/temperance/');
}
...
function render() {
  ...
  const nextCanonicalSlug = CANONICAL_CARD_ORDER[CANONICAL_CARD_ORDER.indexOf('temperance') + 1] || '';
  ...
  if (footerNext) {
    const fallbackNext = localize('/cards/the-devil/index.html');
