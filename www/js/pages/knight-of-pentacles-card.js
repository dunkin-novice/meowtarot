...
const CARD_ID_PREFIX = '76-knight-of-pentacles';
...
function getPagePath(lang = 'en') {
  return getCanonicalCardPath('knight-of-pentacles', lang) || (lang === 'th' ? '/th/cards/knight-of-pentacles/' : '/cards/knight-of-pentacles/');
}
...
function render() {
  ...
  const nextCanonicalSlug = CANONICAL_CARD_ORDER[CANONICAL_CARD_ORDER.indexOf('knight-of-pentacles') + 1] || '';
  ...
  if (crumbMeanings) crumbMeanings.setAttribute('href', localize('/tarot-card-meanings/index.html'));
  if (footerHome) footerHome.setAttribute('href', localize('/'));
  if (footerIndex) footerIndex.setAttribute('href', localize('/tarot-card-meanings/index.html'));
  if (footerNext) {
    const fallbackNext = localize('/cards/queen-of-pentacles/index.html');
...
async function init() {
  ...
  if (!pickCardData()) {
    window.location.replace('/tarot-card-meanings/index.html');
    return;
  }
